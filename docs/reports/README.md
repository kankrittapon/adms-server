...
- [**ADMS-UX-CrossLifecycleClosure-021B**](ADMS-UX-CrossLifecycleClosure-021B.md) — **latest, implementation complete, committed but NOT deployed.** Server-derived `Enrollment.lifecycle_state` (no migration, computed at read time) so the Active Enrollment Queue/Personnel/history views can never disagree about whether an enrollment is genuinely unfinished — self-heals the real production Enrollment #4 row (still `READY_FOR_MAPPING` pending 021's deploy, mapping since closed by terminal removal) into the correct historical state without a backfill. New `Human.has_active_terminal_account` field powers a "no scanner account yet" Personnel card, distinct from the Human being inactive. 685 tests passing.
- [ADMS-UX-FinalPolish-021](ADMS-UX-FinalPolish-021.md) — **implementation complete, committed but NOT deployed.** Auth role-flicker/stuck-stale-role fix (`AuthProvider` loading/error state, deterministic `/auth/me` refetch on login and logout). Enrollment completion semantics fixed with zero migration — `create_verified_mapping()` now atomically drives the enrollment's already-existing `RETIRED` terminal state, which the frontend's completion UI was already built to render. Rank confirmed import-only (Excel-owned); no write endpoint added, source-managed label added instead. 662 tests passing.
