...
- [**ADMS-TerminalManagement-020**](ADMS-TerminalManagement-020.md) — **latest, REVISION complete, committed but NOT deployed.** Full physical fingerprint/terminal-account lifecycle management (`app/terminal_management.py`), all device I/O through the existing DeviceOwner single-owner path, idempotent, ADMIN + write-session gated. Frontend UI complete (`frontend/src/pages/TerminalManagement.tsx`, ADMIN-only). Fingerprint re-enrollment complete via a new dedicated Collector state (`State.FINGERPRINT_ENROLLING`) that suspends live capture at a safe boundary for pyzk's uniquely-blocking `enroll_user()` (~60-180s) and always returns to `LIVE`; mid-call cancellation is documented as impossible with the installed pyzk, not worked around. Cancelled-ID reclamation complete with no migration — proven safe against production data (terminal ID 1002 permanently excluded since it had a real account; 1003 reclaimable since it never did). Read-only ground truth reconfirmed for terminal IDs 1001-1004. 631 tests passing.
