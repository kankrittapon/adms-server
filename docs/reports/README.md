...
- [**ADMS-CurrentState-History-UXClosure-022**](ADMS-CurrentState-History-UXClosure-022.md) — **latest, implementation complete, committed but NOT deployed.** Fixes two confirmed production UX defects presenting historical DB rows as current state: Dashboard's device-user/mapping KPI cards showed historical totals as the headline number instead of current-only counts; the Mapping page showed closed mappings (Pimai/#2) identically to current ones. New canonical `Mapping.mapping_lifecycle_state`/`is_current` (server-derived, no migration) splits the page into Current/History sections. 704 tests passing.
