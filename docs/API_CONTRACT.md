...
| POST | `/api/v1/humans/import/commit` | **gated + ADMIN** — re-validates and re-uploads the same file (never trusts a stale client-held preview); writes only NEW/UPDATE rows; matches existing rows solely by `employee_id` (never by `personnel_id`, never by name) |

**CSV identity model (`ADMS-PersonnelIdentity-AttendanceClosure-025`, corrects `ADMS-Personnel-MasterData-024`)**: `personnel_id` is an optional business field, never required and never used for row matching. CSV round-trip matching uses `employee_id` — the system's own canonical UUID, present only in ADMS's own export (`GET /humans/export.csv`). A row with no `employee_id` is always classified NEW (never required to carry a `personnel_id`); an `employee_id` present but not matching any Human becomes a friendly `ERROR`, never a silent create/ignore; a NEW row whose `display_name` matches an existing Human gets a non-blocking advisory (`warning_th`), never an automatic merge. The downloadable new-person template (`GET /humans/import/template.csv`) omits `employee_id` entirely and never requires `personnel_id`. All 120 legacy production Humans (0 with `personnel_id`) are fully CSV-round-trip-editable via `employee_id` with zero backfill required.
