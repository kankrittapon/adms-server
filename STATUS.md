...
**Latest PromptID**: `ADMS-UX-FinalPolish-021` — implementation complete, commit pushed, **NOT deployed** — pending owner deployment gate; see [docs/reports/ADMS-UX-FinalPolish-021.md](docs/reports/ADMS-UX-FinalPolish-021.md). Fixes: (A) auth role-badge flicker/stuck-stale-role bug — `AuthProvider` now exposes a distinct loading/error state, `Login`/`logout` deterministically trigger an `/auth/me` refetch instead of relying on a hard refresh; (B) Enrollment now visibly completes — `create_verified_mapping()` atomically drives the enrollment's already-existing `RETIRED` terminal state (no migration; state already existed, was just never driven), idempotent on duplicate Step 6; (C) Personnel rank confirmed import-only (no write endpoint added, per source-ownership rule), now labeled as source-managed in the UI. 662 tests passing (+30). No production mutation performed this pass.
