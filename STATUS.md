...
**Latest PromptID**: `ADMS-TerminalManagement-020` (committed, **NOT deployed** — pending owner deployment gate; see [docs/reports/ADMS-TerminalManagement-020.md](docs/reports/ADMS-TerminalManagement-020.md)). Backend architecture for physical ZEM560 fingerprint/account removal — `app/terminal_management.py`, all device I/O routed through the existing DeviceOwner single-owner path, read-before-write/read-after-write verified mutations, idempotent, ADMIN + write-session gated, atomic Human/attendance/mapping/enrollment history preservation. **Frontend UI not yet built** (flagged explicitly in the report). Real finding: pyzk's `enroll_user()` blocks the calling thread for up to 60s interactively — fingerprint re-enrollment is deliberately NOT implemented as an active mutation pending a Collector state-machine design decision. No DB migration. No real terminal user/fingerprint was mutated during this work.
