...
**Latest PromptID**: `ADMS-TerminalManagement-020` — REVISION complete (owner decision C), **NOT deployed** — pending fresh owner deployment gate; see [docs/reports/ADMS-TerminalManagement-020.md](docs/reports/ADMS-TerminalManagement-020.md). Physical ZEM560 fingerprint/account lifecycle management, all device I/O routed through the existing DeviceOwner single-owner path, read-before-write/read-after-write verified mutations, idempotent, ADMIN + write-session gated, atomic Human/attendance/mapping/enrollment history preservation. **Frontend UI complete** (`frontend/src/pages/TerminalManagement.tsx`, `/terminal-management`, ADMIN-only). **Fingerprint re-enrollment complete**: pyzk's `enroll_user()` blocks the calling thread for up to ~60-180s interactively, so a new dedicated Collector state (`State.FINGERPRINT_ENROLLING`) suspends live capture at a safe boundary, runs the call exclusively, and always returns to `LIVE`; result is polled via the existing health-bridge file. Mid-call cancellation is not possible with the installed pyzk and is documented, not worked around. **Cancelled-ID reclamation complete**, no migration: a `CANCELLED` reservation is only reusable when it never had a real terminal account (`terminal_created_at IS NULL AND device_uid IS NULL`) — proven against production data (1002 stays permanently excluded, 1003 becomes reclaimable). 631 tests passing. No real terminal user/fingerprint was mutated during this work.
