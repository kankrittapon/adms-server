...
**Latest PromptID**: `ADMS-Personnel-Lifecycle-019` (committed, **NOT deployed** — pending owner deployment gate; see [docs/reports/ADMS-Personnel-Lifecycle-019.md](docs/reports/ADMS-Personnel-Lifecycle-019.md)). Adds explicit ACTIVE/INACTIVE Human lifecycle (deactivate/reactivate), atomic mapping closure on departure (`valid_to` = departure time, historical mappings/attendance never touched), Personnel list active/inactive filtering, and Thai lifecycle UX. No DB migration (`human_employees.active` already existed). Terminal/fingerprint cleanup is explicitly out of scope — deferred to `ADMS-TerminalManagement-020`. The `admin` operator account was verified already `ADMIN` — no change made.
...
- **Automated Test Baseline**: **574 passed / 0 failed** (`pytest tests/`, includes the write-session, terminal-account-idempotency, DeviceCommandBus timeout-margin, single-owner device I/O, mapping-evidence, full Step 1→6 E2E, controlled-scan evidence binding, and Personnel lifecycle matrices)
