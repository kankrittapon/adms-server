...
21. **`ADMS-Data-DeviceEnrollmentWorkflow-002`**: Device Enrollment Workflow Implementation (**PENDING — WRITE mode, requires explicit owner approval** — Implement enrollment infrastructure: ID allocator, reservation table, safe `set_user()` account creation, roster verification. **Requires DeviceUserLifecycle-002 first.**).
22. **`ADMS-Data-HumanDeviceMapping-003`**: First Production Enrollment / Human ↔ Device Mapping WRITE (**PENDING — WRITE mode, requires explicit owner approval** — Execute pilot enrollment for 1 Human: allocate ID, create account, enroll fingerprint, controlled scan, create VERIFIED mapping. **Requires DeviceEnrollmentWorkflow-002 first.**).
23. **`ADMS-Data-HumanDeviceMapping-004`**: Post-Mapping Checkpoint (**PENDING — READ-ONLY** — Verify pilot mapping creation, temporal resolver behavior, reconciliation correctness, runtime health).
24. **Native ADMS Push E2E**: EXPERIMENTAL TRACK ONLY (Isolated verification after identity workflow foundation is complete).
...
| `ADMS-Data-DeviceUserLifecycle-002` | 2026-08-12 | Device User Lifecycle Implementation | COMPLETE (Latest Task) | WRITE mode. Implemented `reconcile_roster_lifecycle()` in `app/db.py` (atomic per-device roster reconciliation: observed users → ensure + roster_last_seen_at + clear inactive_at, missing users → set inactive_at if NULL, UID change detection, no deletions). Added `perform_roster_lifecycle_check()` in `app/collector.py` (roster snapshot in BACKFILLING, periodic polling in LIVE every `roster_poll_interval_seconds` default 300s, failure-safe: exception/None → no lifecycle updates). Added `roster_poll_interval_seconds` to `app/config.py`. 18 new tests (105/105 total pass). Pre-write backup verified (45049 bytes, 79 TOC entries). Deployed to ai-brain (listener rebuilt only, PostgreSQL/MQTT untouched). First poll LIVE VERIFIED: 0 terminal users → both device_users (id=1, id=2) marked inactive_at=2026-08-12 04:31:54+00, active=false. Row counts unchanged (120/2/0/7, sync_events +1 ROSTER_LIFECYCLE). 0 deletions, 0 mappings, 0 human_employees created, 0 schema changes, 0 terminal writes. Post-write backup verified (45172 bytes). |

---

## Pending & Upcoming Work

1. **Device Enrollment Workflow Implementation**:
   - `# PromptID: ADMS-Data-DeviceEnrollmentWorkflow-002` (WRITE mode — **REQUIRES EXPLICIT OWNER APPROVAL** — Implement minimum infrastructure for controlled enrollment: ID allocator (1001+), reservation mechanism (`enrollment_reservations` table), safe account creation path (`set_user()` wrapper with audit), roster verification. **DeviceUserLifecycle-002 COMPLETE — unblocked.**)

2. **First Production Enrollment (Pilot)**:
   - `# PromptID: ADMS-Data-HumanDeviceMapping-003` (WRITE mode — **REQUIRES EXPLICIT OWNER APPROVAL** — Execute pilot enrollment for 1 Human: allocate ID, create terminal account, enroll fingerprint, controlled scan, create VERIFIED mapping. **Requires DeviceEnrollmentWorkflow-002 first.**)

3. **Post-Mapping Checkpoint**:
   - `# PromptID: ADMS-Data-HumanDeviceMapping-004` (READ-ONLY — Verify pilot mapping creation, temporal resolver behavior, reconciliation correctness, runtime health.)
