...
**Latest PromptID**: `ADMS-CurrentState-History-UXClosure-022` — implementation complete, commit pushed, **NOT deployed** — pending owner deployment gate; see [docs/reports/ADMS-CurrentState-History-UXClosure-022.md](docs/reports/ADMS-CurrentState-History-UXClosure-022.md). Fixes two confirmed production UX defects where historical DB rows were presented as current state: (A) Dashboard's headline "device users" KPI showed `device_users_total` (5, includes historical/removed rows) instead of `device_users_active` (1, currently on the scanner) — same defect found and fixed for the Mappings KPI card too; (B) the Mapping page showed closed/historical mappings (e.g. Pimai's #2) with the identical green "VERIFIED" badge as current ones — now split into visually separated Current/History sections via a new canonical `Mapping.mapping_lifecycle_state`/`is_current` field (server-derived, no migration, no duplicated logic across pages). 704 tests passing (+12). No production/device mutation performed this pass.
