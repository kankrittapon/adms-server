...
20. **`ADMS-Data-DeviceUserLifecycle-002`**: Device User Lifecycle Implementation (**NEXT — WRITE mode, requires explicit owner approval** — Implement automatic roster lifecycle detection: periodic `get_users()` comparison, populate `roster_last_seen_at`/`inactive_at`, set `active = false` on disappearance. **PREREQUISITE for production enrollment.**).
...
5. **Parse_Time Bug Fix (NON-BLOCKING, recommended)**:
