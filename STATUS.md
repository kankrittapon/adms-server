...
**Latest PromptID**: `ADMS-UX-CrossLifecycleClosure-021B` — implementation complete, commit pushed, **NOT deployed** — pending owner deployment gate; see [docs/reports/ADMS-UX-CrossLifecycleClosure-021B.md](docs/reports/ADMS-UX-CrossLifecycleClosure-021B.md). Closes a gap 021 didn't cover: a server-derived `Enrollment.lifecycle_state` (`IN_PROGRESS`/`COMPLETED`/`REMOVED_FROM_TERMINAL`/`CANCELLED`, computed at read time via SQL join, no migration) so the Active Enrollment Queue, Personnel, and history views can never disagree about whether an enrollment/terminal account is genuinely unfinished — self-heals even the real production Enrollment #4 row (still `READY_FOR_MAPPING` in the DB pending 021's deploy) into the correct historical state. Personnel now shows a distinct "ยังไม่มีบัญชีบนเครื่องสแกน" (no scanner account) card for an ACTIVE Human whose terminal account was removed, via a new `Human.has_active_terminal_account` field — never conflated with the Human being inactive. 685 tests passing (+23). No production mutation performed this pass.

**Also implementation-complete, NOT deployed**: `ADMS-UX-FinalPolish-021` — see [docs/reports/ADMS-UX-FinalPolish-021.md](docs/reports/ADMS-UX-FinalPolish-021.md). Fixes: (A) auth role-badge flicker/stuck-stale-role bug — `AuthProvider` now exposes a distinct loading/error state, `Login`/`logout` deterministically trigger an `/auth/me` refetch instead of relying on a hard refresh; (B) Enrollment now visibly completes — `create_verified_mapping()` atomically drives the enrollment's already-existing `RETIRED` terminal state (no migration), idempotent on duplicate Step 6; (C) Personnel rank confirmed import-only (no write endpoint added, per source-ownership rule), now labeled as source-managed in the UI.
...
- **Automated Test Baseline**: **597 passed / 0 failed** (`pytest tests/`, includes the write-session, terminal-account-idempotency, DeviceCommandBus timeout-margin, single-owner device I/O, mapping-evidence, full Step 1→6 E2E, controlled-scan evidence binding, Personnel lifecycle, and Terminal Management matrices)
