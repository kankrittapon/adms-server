...
echo "=== Wait 30s for startup + backfill ==="
sleep 30
...
echo "=== Wait more for healthcheck ==="
sleep 35

echo "=== Health status ==="
docker inspect adms_zkteco_listener --format 'restart_count={{.RestartCount}} state={{.State.Status}} health={{.State.Health.Status}}' 2>&1
docker inspect adms_zkteco_listener --format '{{range .State.Health.Log}}{{.ExitCode}} {{.Output}}{{end}}' 2>&1
...
docker exec adms_postgres psql -U adms -d adms -c "SELECT user_id, device_ip, scan_time, punch_type, status, device_id, device_user_pk, employee_id FROM attendance_logs ORDER BY scan_time LIMIT 20;"

echo "=== Unmapped attendance ==="
docker exec adms_postgres psql -U adms -d adms -t -c "SELECT count(*) FROM attendance_logs WHERE employee_id IS NULL;"

echo "=== sync_events ==="
docker exec adms_postgres psql -U adms -d adms -c "SELECT event_type, message, created_at FROM sync_events ORDER BY created_at DESC LIMIT 10;"
